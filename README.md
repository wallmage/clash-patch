...
Clash Patch 改用三个直接连接 IP 的 DoH 地址：AdGuard 的 `94.140.14.14`、`94.140.15.15`，以及 TWNIC Quad 101 的 `101.101.101.101`。它们仍然通过原主代理组访问。已有 `nameserver-policy` 的分流目标会保留，但解析器地址会换成这组受管地址。
