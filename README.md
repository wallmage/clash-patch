...
加密解析器仍可使用 `https://1.1.1.1/dns-query` 等地址，但它们带原主代理组标签，经代理访问。负责解析代理节点域名的 `proxy-server-nameserver` 优先保留订阅原值；字段缺失或等于旧版补丁写入的固定境外组合时改用 `system`。`default-nameserver` 和 `direct-nameserver` 保持原值；只迁移旧版补丁写入的 `1.1.1.1`、`8.8.8.8` 组合。
