...
- 创建 `🛡 安全代理 · Clash Patch`，让普通查询和 AI 查询通过带代理组标签的加密 DNS。
- 创建 `🤖 AI · Clash Patch`，路由 OpenAI、ChatGPT、Codex、Claude、Anthropic 和已收录的 Google AI 服务。
- 台湾家宽优先，日本家宽其次；没有两者时继续跟随原主代理组。
- 把通用 UDP 先交给安全代理组，下一条规则立即拒绝失败流量，避免退回 `DIRECT`。
...
加密解析器仍可使用 `https://1.1.1.1/dns-query` 等地址，但它们带安全代理组标签，经代理访问。负责解析代理节点域名的 `proxy-server-nameserver` 优先保留订阅原值；字段缺失或等于旧版补丁写入的固定境外组合时改用 `system`。`default-nameserver` 和 `direct-nameserver` 保持原值；只迁移旧版补丁写入的 `1.1.1.1`、`8.8.8.8` 组合。

这条边界由 Ruby、JavaScript 和策略一致性测试共同检查。
