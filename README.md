...
安装程序会按 YAML 的映射层级修改 `config.yaml`，能正确处理 `tun: null`、带空格的键和块状 `dns-hijack`。三个目标文件先全部生成并检查，再统一写入；中途任何一步失败，已经写过的文件会恢复到本次安装前的内容。

如果原本已有全局扩展脚本，安装程序会先备份，再尝试组合：先运行原脚本，再运行 Clash Patch。无法安全组合时，它会保留原文件，并请你提供报错信息或截图。

## 移除自动补丁

移除操作只关闭“订阅刷新后自动再打补丁”，不会贸然还原当前订阅，也不会删除备份。
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\clash-patch\scripts\uninstall_windows.ps1
```
...
- 仓库包含 Ruby、Node 和 PowerShell 回归测试；GitHub Actions 会在 Linux 和 Windows 上自动运行。
