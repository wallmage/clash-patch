...
- 档位 3 且客户端原本没有运行时，安装程序才事务式更新应用级设置：`verge.yaml` 的 `enable_tun_mode` 设为 `true`、`enable_dns_settings` 设为 `false`，`config.yaml` 关闭 `ipv6` 并写入托管 TUN 块（保留非托管的 TUN 设置和行内注释）；同时把两份文件的原始字节和安装后哈希写入本地安装状态文件，供卸载核对。全部目标先备份，再持有禁止外部写入或替换的句柄，确认仍与候选生成时完全一致后统一写入；任一失败整体恢复。
