...
Windows 中断的当前配置事务恢复前也会检查客户端。恢复计划需要修改根目录 `config.yaml` 或 `verge.yaml` 时，客户端运行中返回 `partial/transaction_recovery_pending`，保持中断字节、事务记录和新建文件准备记录不变；以后再次运行任一公开命令时继续恢复。只涉及脚本或状态文件的事务不受此限制。
