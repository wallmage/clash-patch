...
Diagnostics 不要求用户会看日志，也不会因为提到 Clash 就先改配置。它按现场选择需要检查的浏览器、应用、DNS、连接、分流、传输质量、系统记录和日志；证据不足时建立一段有时间范围的观察。浏览器白屏或一直转圈时，会把主文档网络耗时和页面首次显示分开测量；主文档很快而页面仍空白时，再对扩展做单站停用的对照测试，无改善就恢复原设置。能安全自动修复的由 AI 助手完成，本机解决不了的会先确定问题类别，再查找当前的官方方案。
