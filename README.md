...
- macOS：先按所有权状态恢复并回读本工具关闭的订阅自动更新；恢复失败时保留用途档位和所有权状态，不继续假装降档完成。随后以可恢复的整批删除事务移除安装文件，并移除能确认所有权的旧版监听。旧版 TUN 启动偏好记录无法证明仍代表用户当前选择，因此卸载时保留当前值。订阅里的 DNS、WebRTC 和 AI 设置不会自动撤销，卸载输出会如实说明。
