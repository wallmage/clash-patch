...
2. 为每份当前配置创建带日期时间的修改前备份。macOS 运行 `bash scripts/install_macos.sh --safe-update`，一次下载全部远程订阅；全部下载、YAML 重读、二次转换和 Mihomo 校验成功后才统一替换。任何一份失败时全部保持原样。
3. 档位 3 对每份下载结果重新应用完整补丁；档位 1、2 不增加第三档设置。更新后刷新当前订阅并复核运行状态，失败时整批回滚。
4. Windows 先运行 `.\scripts\install_windows.cmd -SnapshotProfiles` 为全部订阅备份，再用 Computer Use 确认清单并在 Clash Verge Rev 中执行“全部更新”；档位 3 的全局脚本会对每份加载结果重打补丁。客户端没有“全部更新”时逐份触发，但必须覆盖清单中的每一份并逐份报告。
