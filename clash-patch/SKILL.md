...
补丁使用自己管理的 `🤖 AI · Clash Patch` 和 `🛡 安全代理 · Clash Patch`，不修改用户同名组或用户规则的目标。安全组不含 `DIRECT`、`DNS`、`REJECT`、`PASS`、`COMPATIBLE`、`REMATCH` 或同类出口。

普通查询和 AI 域名查询使用带安全组标签的加密解析器。启动解析字段保持订阅原值；仅在 `proxy-server-nameserver` 缺失或仍是旧版危险固定值时使用 `system`。这样可以避免代理节点域名依赖代理本身才能解析。

通用 UDP 先交给安全代理组，紧接 `NETWORK,UDP,REJECT`。这会影响 WebRTC、游戏、语音、视频通话和 QUIC，执行前必须告诉用户。

台湾家宽优先，日本家宽其次。补丁 AI 组只有一个合格成员；两者都没有时跟随主代理组，不选择其他地区。文件写入后只能说：

> 重新加载或选择该订阅后，AI 流量将使用「{节点名称}」。

本 skill 不自动重新加载或切换节点，所以不得发送“已切换”通知，也不得承诺一定可用、永不风控或绝对匿名。
...
同时说明主代理组、补丁 AI 组、安全代理组、家宽选择、TUN、是否安装自动机制以及三项测试结果。不要修改或添加个人化的 Apple、iCloud 或测速网站规则。
