...
档位 1、2 的安装程序只保存选择，不改订阅；客户端开关由 Computer Use 操作并复测。档位 3 先通过客户端界面关闭全部订阅的自动更新，再继续完整补丁；安装程序会拒绝在无法确认自动更新已关闭时改订阅。macOS 只单次处理当前存储位置，不安装永久监听；当前订阅修改后自动刷新，并检查 TUN、DNS、外网连通性和原有代理组选择，失败时自动恢复。Windows 使用全局扩展脚本；客户端运行时只更新 `profiles/Script.js`。两个平台都必须让 Clash 保持运行。
