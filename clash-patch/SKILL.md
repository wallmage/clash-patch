---
name: clash-patch
description: Use when an agent needs to diagnose, explain, and repair slow, intermittent, unavailable, misrouted, or leaking network behavior, or safely apply DNS, WebRTC, TUN, and AI routing patches for ClashX Meta or Clash Verge Rev on macOS or Windows.
---
...
2. 先只读检查，再修改。macOS 修改当前订阅后必须通过本地 Mihomo 控制器安全刷新并验证；不得切换 TUN、订阅、代理组或节点。
...
- **Patch 模块**：用户首次安装或明确要求应用完整安全增强时使用。
- **Diagnostics 模块**：用户说网络不对劲、变慢、打不开、不稳定、分流异常或出现其他网络现象时使用。不能因为用户提到 Clash 就先运行补丁。

Diagnostics 默认只读。先复现原始症状并判断影响范围，再按现场选择浏览器、应用、系统、DNS、连接、分流、传输质量和日志证据；不是每次全部检查。保留多个解释，用对照和反证逐个验证。没有证据不能改配置，也不能把可能性写成结论。

浏览器白屏、一直转圈或过很久突然显示时，必须把 DNS、主文档与关键资源的网络耗时、首次显示时间和加载结束时间分开记录。主文档和资源很快而页面仍空白时，不得继续归咎 DNS 或 Clash；检查脚本执行、Service Worker 和扩展注入，并优先做单站停用扩展的对照。只有原始症状明确改善才保留该修复；无改善立即恢复原设置，不得停用整个扩展或影响其他网站。

代理必须主动完成本机可完成的检查。历史证据不足时建立有时间戳的观察窗口，不让小白用户决定该查哪个日志。确认问题后执行最小、可恢复的修复，并针对原始症状复测；本机不能解决时，先用本机证据确定问题类别，再在线查找当前的官方解决办法。最终用简体中文说明发生了什么、证据、已做修复、复测结果和仍未确认之处。

单项 Clash 配置修复仍留在 Diagnostics，只复用 Patch 的备份、候选校验、刷新和失败恢复机制。完整增强才进入 Patch 并执行下述完整策略与 Patch 专用验收。普通浏览器或应用问题不得顺手改 Clash。
