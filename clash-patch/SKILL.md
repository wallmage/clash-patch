...
普通查询和 AI 域名查询统一使用带主代理组标签、以 IP 直接连接的加密 DoH。不要使用 `dns.google` 等需要先解析域名的解析器，也不要继续使用受部分节点限制的 Google/Cloudflare 固定地址。已有 DNS 分流目标可以保留，但解析器地址要换成策略文件中的受管地址。启动解析字段保持订阅原值；仅在 `proxy-server-nameserver` 缺失或仍是旧版危险固定值时使用 `system`，避免节点域名依赖代理本身才能解析。
