...
只有确认没有可选 AI 分组时，才创建 `🤖 AI · Clash Patch`。新组只包含原主代理组，让 AI 流量跟随用户原来的主节点选择。名称被其他代理组或内联代理占用时使用没有冲突的编号。重复运行必须复用已创建的组，不能产生重名。

升级旧版补丁时：订阅已有自己的 AI 分组，就删除能确认属于旧补丁的重复 AI 组并把补丁规则改到原有 AI 分组；订阅没有其他 AI 分组，就保留旧补丁 AI 组，但把成员改成原主代理组。不得自动放入任何具体节点。

AI 组负责 OpenAI、ChatGPT、Codex、Claude、Anthropic，以及策略文件中列出的 Google AI 和相关服务流量。

## 节点建议

补丁不得修改 AI 分组成员，不得调用控制器切换节点，也不得替用户选择台湾、日本或任何家宽节点。
