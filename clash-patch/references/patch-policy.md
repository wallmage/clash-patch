...
| 档位 | 用途 | 必须做 | 明确不做 | 验收 |
| --- | --- | --- | --- | --- |
| **档位 1｜普通浏览** | Twitter、Facebook、YouTube 等 | 用 Computer Use 确认 Clash 客户端的“设置为系统代理”已开启 | 档位 1 不修改 TUN，不修改订阅、DNS、WebRTC、AI 分组或节点 | Google、Twitter 和一个用户常用站点能稳定打开，速度无明显异常 |
...
- **档位 1：** 目标是普通境外网站可以通过系统代理稳定浏览。档位 1 不检查或修改 TUN。档位 1 不运行 DNS 泄漏、WebRTC 或 AI 检查，也不查看或修改 AI 分组、Claude 规则和第三档订阅增强。证据指向解析问题时可以检查目标域名的普通 DNS 解析、缓存和连接耗时；这不等于 DNS 泄漏检查，只能修复与原始浏览故障直接相关的部分。
- **档位 2：** 目标增加 ChatGPT、Codex、Gemini、Perplexity 和 Agent 联网，并保持 TUN 开启、Clash 自己的系统代理关闭。可以在症状相关时检查 TUN 和 Agent 连接；档位 2 不运行 DNS 泄漏、WebRTC 或 AI 分组检查，也不安装或改写第三档订阅增强。
...
**Windows 单项配置事务：** Windows 当前没有安全的即时单项配置写入路径。客户端运行时不得改当前配置；全局脚本又代表档位 3 的完整 Patch，不能用于伪装单项修复。完成诊断后报告已确认问题与生效边界；只有用户选择档位 3 或明确要求完整增强时才安装全局脚本，并等待客户端以后正常加载或刷新。
...
以下完整流程只用于档位 3。档位 1、2 在保存选择、完成客户端开关和各自验收后结束。
...
4. 档位 3 对每份候选重新应用完整补丁；档位 1、2不得增加第三档字段。全部候选都要经过 YAML 重读、二次转换一致性检查和受支持 Mihomo 校验。
...
Windows 不安装永久监听、计划任务或后台服务。Clash Verge Rev 运行时通常只允许更新 `profiles/Script.js` 和 `profiles.yaml` 中的 `option.allow_auto_update`，不得修改 `verge.yaml`、`config.yaml` 或当前运行配置。唯一例外是用户明确触发安全更新后，`-VerifySafeUpdate` 可以按本次不可伪造的清单把订阅文件恢复到刚创建且哈希匹配的备份；它仍不能声称运行内核已经恢复，后续必须单独做实时验收。自动更新修改器只处理 `type: remote` 项目；保留本地项目、订阅地址、更新时间、间隔和其他选项。无法唯一识别 `items`、`uid`、`type`、`option` 或目标字段，或遇到锚点、别名、重复键、行内复杂映射、多文档与制表符缩进时直接拒绝。每次实际写入前备份，写入后重新读取并确认所有远程订阅都是 `false`；任一步失败时恢复整个事务。所有档位 3 持久修改前都先确认可信路径中的 Mihomo 版本；客户端未运行时才继续用它检查应用配置候选并修改 TUN、IPv6 等应用设置。安装程序处理 `tun: null`、空格键名、行尾注释和块状 `dns-hijack`；无法安全合并的锚点与多文档 YAML 直接拒绝，`--- # 注释` 也算文档标记。全部候选先生成并检查，再写入；中途失败时继续恢复每一个目标，并按原始字节保留编码和 BOM。Windows 每次写入前创建带日期时间的版本化备份；备份停止继承目录权限，只允许当前用户、SYSTEM 和 Administrators 完全访问。
