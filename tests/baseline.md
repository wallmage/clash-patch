...
- Ruby：macOS 三档用途、全部订阅共享的国内域名规则提供器、名称冲突保护、普通安装整批预检与失败零写入、备份/比较/恢复、安全更新事务、批量更新期间的路径原子替换、中途写入失败恢复、同一 inode 别名拒绝、空配置目录拒绝、运行刷新、恢复当前订阅时保持 TUN 状态并复核运行内核、文件与内核恢复状态分别上报、YAML 1.2、Mihomo 校验和带域名边界的分流验证。
...
- PowerShell 5.1 与 7：Windows 三档用途、安装/卸载、失败时保持原档位、安全卸载后的真实降档、自动更新关闭及嵌套 `option` 层级、远程订阅清单、安全更新校验字节绑定、恢复校验、拒绝覆盖候选生成后的并发改动、事务恢复、公开操作互斥，以及带域名边界的分流验证；全库通过 PowerShell AST 禁止在赋值、参数、循环、自增减或变量命令中写入只读自动变量。
...
- 测试机制：CI 的 macOS 与 Windows job 都有总超时；关键安全判断通过 mutation smoke test 证明相应回归会失败，mutation 的语法错误、加载失败或超时不能冒充被测试发现。
