...
- PowerShell 5.1 与 7：两个 CI 入口都必须由父进程和实际子进程自证 PSEdition 与主版本；覆盖 Windows 三档用途、安装/卸载、失败时保持原档位、安全卸载后的真实降档、远程订阅自动更新的原值与原 `option` 形态所有权、重复安装不改写原状态、严格状态结构与单次 snapshot、运行客户端且存在应用状态时整批零改动、写删重叠和重复目标预检、全目标句柄先取得、文件身份与原字节双重核对、阻断读取/写入/改名/替换、句柄绑定删除、删除标记失败恢复、目录/重解析点/硬链接拒绝、坏备份先拒绝、内部状态恢复禁令、AppHome 公开操作互斥、失败恢复不覆盖并发内容、嵌套 `option` 层级、远程订阅清单、安全更新校验字节绑定，以及同时绑定源端口、TCP、新连接 ID 与域名边界的分流验证；全库通过 PowerShell AST 禁止在赋值、参数、循环、自增减或变量命令中写入只读自动变量。
