...
- PowerShell 5.1 与 7：Windows 三档用途、安装/卸载、失败时保持原档位、安全卸载后的真实降档、自动更新关闭及嵌套 `option` 层级、远程订阅清单、安全更新校验字节绑定、恢复校验、拒绝覆盖候选生成后的并发改动、事务恢复、公开操作互斥，以及同时绑定源端口、TCP、新连接 ID 与域名边界的分流验证；全库通过 PowerShell AST 禁止在赋值、参数、循环、自增减或变量命令中写入只读自动变量。
