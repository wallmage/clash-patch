...
- Ruby：macOS 三档用途、全部订阅共享的国内域名规则提供器、名称冲突保护、普通安装整批预检与失败零写入、备份/比较/恢复、安全更新事务、订阅自动更新所有权记录与安全恢复、批量更新期间的路径原子替换、中途写入失败恢复、同一 inode 别名拒绝、空配置目录拒绝、运行刷新、恢复当前订阅时保持 TUN 状态并复核运行内核、文件与内核恢复状态分别上报、YAML 1.2、Mihomo 校验，以及同时绑定源端口、TCP、新连接 ID 与域名边界的分流验证。
