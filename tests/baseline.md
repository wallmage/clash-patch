...
- Ruby：macOS 三档用途、全部订阅共享的国内域名规则提供器、名称冲突保护、转换、备份/比较/恢复、安全更新事务、运行刷新、恢复当前订阅时保持 TUN 状态并复核运行内核、文件与内核恢复状态分别上报、YAML 1.2、Mihomo 校验和分流验证。
...
- PowerShell 5.1：Windows 三档用途、安装/卸载、自动更新关闭、远程订阅清单、安全更新前后检查、嵌套配置差异、恢复校验、事务恢复，以及 `Observe-Route` 对真实连接对象的运行路径；全库同时禁止写入只读自动变量。
