...
- Ruby：macOS 三档用途、全部订阅共享的国内域名规则提供器、名称冲突保护、普通安装整批预检与失败零写入、备份/比较/恢复、安全更新事务、订阅自动更新分阶段所有权记录与安全恢复、带截止时间的文件锁、同目录原子交换、批量更新期间的路径与文件身份变化、中途写入失败恢复、同一 inode 别名拒绝、空配置目录拒绝、运行刷新、恢复当前订阅时重新清缓存并复核 TUN、代理选择、DNS 与连通性、文件与内核恢复状态分别上报、YAML 1.2、Mihomo 校验，以及同时绑定源端口、TCP、新连接 ID、域名边界并拒绝非代理终点的分流验证。
